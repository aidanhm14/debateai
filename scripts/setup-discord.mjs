#!/usr/bin/env node
// One-shot Discord server setup for the Debate AI community.
...
const ROLE_SPECS = [
  ...
  { name: 'Debate AI Team', color: c('#f97316'), hoist: true, mentionable: true },
...
function channelSpecs(roles) {
  ...
  const teamId = roles.get('Debate AI Team').id;
  ...
  return [
    ...
    },
    {
      ...
      channels: [
        ...
        { name: 'feature-requests', type: TYPE.TEXT, topic: 'What should Debate AI do that it doesn\'t? Upvote with reactions.' },
      ],
    },
    {
...
// ────────────────────────────────────────────────────────────────────
const WELCOME_MESSAGE = `# Welcome to Debate AI.
...
async function setWelcomeScreen(channels) {
  ...
  const body = {
    ...
    welcome_channels: [
      ...
      { channel_id: channels.get('feedback').id, description: 'Make Debate AI better. The team reads everything.', emoji_name: '🛠️' },
    ],
  };
