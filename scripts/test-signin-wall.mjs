...
p = page(); p.advance(18); p.leave(); p = page({ path: '/practice', storage: p.storage }); p.advance(21.75); assert.equal(p.asks.length, 0); p.advance(.25); assert.equal(p.asks.length, 1);
