...
assert.equal(firstTrack.children.length, payload.messages.length, 'The full history is visible before the first animation tick');
assert.deepEqual(firstTrack.children.map(row => row.children[1].textContent).sort(), payload.messages.map(row => row.text).sort(), 'Initial messages are complete');
assert.equal(list.scrollTop, list.scrollHeight, 'The loaded chat starts at the bottom');
const firstRow = firstTrack.children[0], firstText = firstRow.children[1];
tick();
assert.equal(firstTrack.children.at(-1), firstRow, 'The continuing sequence enters at the bottom');
...
tick();
assert.equal(list.scrollTop, 0, 'Typing does not pull someone reading older messages back down');
...
context.window.__fsChatsCache = payload.messages;
nextPayload = new Error('Network unavailable');
vm.runInContext(code, context);
assert.equal(list.children[0].children.length, payload.messages.length, 'A cached visit paints all history synchronously');
assert.ok(list.children[0].children.every(row => row.children[1].textContent.length > 0), 'Cached history never starts as empty typing rows');
await new Promise(resolve => setImmediate(resolve));
assert.equal(panel.classList.contains('is-live'), true, 'A failed refresh keeps the preloaded history');
