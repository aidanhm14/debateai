...
export default async (request) => {
  ...
  //
  // Fixed 2026-08-10: this read (and visitor-tick's matching write)
  // used `metrics/daily/{date}`, three segments, which Firestore
  // rejects as a document path. Both threw on construction for
  // months, so totalVisits was structurally pinned at 0. Now a flat
  // 2-segment `metrics_daily/{date}`. Days before the fix have no
  // docs and read as 0, because none were ever written.
