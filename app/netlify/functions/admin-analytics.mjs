...
export default async (request) => {
  ...
  // Admin check: must match hardcoded UID or have admin flag in Firestore
  ...
  try {
    ...
    // Team metrics (need full docs for plan/usage analysis)
    ...
    try {
      ...
      // createdAt field or index may not exist; fall back to 0
      console.warn('Could not count recent signups:', err.message);
    }

    // Top features from learning_counters aggregation
    let topFeatures = [];
    try {
