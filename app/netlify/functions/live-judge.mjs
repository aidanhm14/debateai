...
export function buildPrompt(d) {
  ...
  const conversation = d.format === 'open' || d.format === 'conversation';
  const system = [
    ...
    conversation ? 'OPEN CONVERSATION: questions and answers count directly wherever they occur. There are no timed speeches or overtime deductions. Apply the conversation method to interruptions and unfinished exchanges. WORDS MEAN WHAT THE ROOM SAID THEY MEAN: in a conversation nobody defines terms at the top, so a meaning arrives mid-stream ("when I say union I mean any collective mobilisation"). Judge each person on the meaning they stated. If the other person acknowledged it and kept arguing ("I know you define it differently, but..."), that meaning stands for both sides. If they contested it with a reason, decide which reading fits the topic as written and say so. Never quietly substitute a dictionary, textbook, or your own definition for one the room used, and when the two disagreed about a key word, the decision must name the meaning that governed and why.' : 'INTERJECTIONS: an [Interjection] line inside a speech is the other side speaking during it, a point of information, a question, the back and forth. Judge the exchange fairly, from both directions: a concise, relevant question that exposes a real gap is engagement and may lift the asker\'s responsiveness and strategy, and a clean answer under pressure lifts the speaker. Score an interjection only when its substance is identifiable. Heckling, badgering, repeated interruption, or talking over an answer lowers the INTERRUPTER\'s responsiveness, strategy, or persuasion; never penalize a speaker merely for being interrupted, and never treat holding the floor through noise as a weakness.',

    conversation ? '' : 'TIME DISCIPLINE: a [Time: ...] line under a speech means that speaker ran meaningfully past their allotted time; a 15 second grace is already excluded, so a speech with no time line was on time and gets no deduction. After computing that side\'s points from the axes, subtract for the overrun and say so in the RFD or feedback: about 2 of 100 up to 45 seconds over, 4 up to 90 seconds, 5 beyond that. A line marked "length unknown" is at most a 1 point deduction. A [TIME EXPIRED] marker inside a speech is a hard cutoff: everything below it is OFF THE FLOW — it cannot win an argument, extend one, or count as an answer to one, and the RFD may mention it only to note it came after time. A mutually agreed extension already moved the allotted time, so an extended speech with no time line was on time. Overtime moves speaker points, never the winner.',
    ...
    '  "rfd": "<14-22 sentences. OPEN WITH THE CALL: sentence one names the winner and the ground they won it on, sentence two names what the losing side rested on and why it was not enough. Those first two sentences are shown ALONE as the short version of this ballot, so a reader who reads only them must come away knowing who won and why; an opening that states the clash neutrally without resolving it reads to the loser as though they won. Then the decision, issue by issue and on the flow. Name each clash that mattered, say who took it and on what, and name every consequential drop only when the other side extended it and explained why it mattered to the ballot. Quote the line that settled a clash where a line settled it, then close with the single thing the losing side needed to change. Do not summarise the speeches back; a debater already knows what they said and is reading this to find out what it was worth.>",',
