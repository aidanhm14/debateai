...
export default async (request) => {
  ...
  try {
    ...
    // ── Mode C: incremental transcript (WS1 resilience) ───────────────
    if (action === 'incremental_transcript' || body.kind === 'voice_round_incremental') {
      const { generationId, turns: turnsToWrite } = body;
      if (!generationId || typeof generationId !== 'string') {
        return errorResponse('Missing generationId for incremental transcript', 400, request);
      }
      if (!Array.isArray(turnsToWrite) || turnsToWrite.length === 0) {
        return errorResponse('No turns to write', 400, request);
      }
      ...
      for (let i = 0; i < turnsToWrite.length; i += 500) {
        const batch = db.batch();
        const turnsBatch = turnsToWrite.slice(i, i + 500);

        for (const turn of turnsBatch) {
          if (turn && turn.id) {
            const turnRef = turnsCollRef.doc(turn.id);
            batch.set(turnRef, {
              ...turn,
      ...
      return jsonResponse({ ok: true, turnCount: turnsToWrite.length }, 200, request);
    }
