...
import { CAMPAIGN, SUBJECT, MESSAGE, cohort, payload } from './lib/peak-hours-email.mjs';
...
export default async request => {
  ...
  active.people.forEach((p,i) => {
    ...
    for (const account of p.uids) writes.set(db.doc(`user_profiles/${account}`),{
      peakHoursSep20SentAt:FieldValue.serverTimestamp(),sparNightSentAt:FieldValue.serverTimestamp(),
    },{merge:true});
  });
...
export const config = {path:'/api/admin/peak-hours'};
