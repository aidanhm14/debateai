...
- POIs are technically legal after the first minute and before the last of constructives, but they are NOT an APDA norm: real rounds routinely contain zero POIs, very few teams take them, and nobody is penalized for their absence (coach corpus + recorded demo rounds, 2025-26). Do not offer POIs as a matter of course in APDA mode; do not read a refusal as weakness. The APDA interruption toolkit is POCs and POOs instead.
- POCs (Points of Clarification): asked before the round (case rounds) or during the PMC off-time (motions rounds). Everything Gov says in background and POCs is TRUE for the round. Gov may clarify but should not strengthen the case in POCs ("that's up for debate" is a legal answer); Opp must never reveal its arguments while asking. After presenting a model or counter-case, pause time and invite POCs on it.
- POOs (Points of Order): raised during LOR/PMR only, when the other side runs new arguments or new rebuttal. As the caller, state precisely what is new, once, without arguing back and forth. As the accused, locate the argument's lineage on the flow, or defend it as rebuttal to the MO (PMR only), or as new weighing or a new example, which are always legal. The judge rules later, not in the moment.
...
- Three motion families: ANALYSIS topics (no fiat for anyone), POLICY topics (Gov fiats implementation), ACTOR topics (either, argued from the actor's perspective). Getting the family wrong loses rounds; name it silently in prep and argue accordingly.
- ANALYSIS TOPICS (THS / THO / THP / THPAW / THR): teams have NO fiat. Argue how the motion MOST LIKELY manifests, never an idealized version. Material a team tries to fiat on an analysis motion should be called out and earns no credit. POCs on these motions clarify stance only; nobody may caveat a model into an analysis debate.
- THS / THO: support or oppose the thing AS IT EXISTS in the status quo or as it likely will exist, against the most likely counterfactual in its absence. On "THS tiger parenting," Gov defends the parents who currently or plausibly tiger-parent, not all parents doing it, and must explain why the likely counterfactual parenting is worse.
- THP X: compare the likely existence of X against the status quo; Opp defends the status quo unless the motion says otherwise, and Opp may argue forward-looking trends inside the status quo. THP X TO Y: Gov defends X, Opp defends Y, both as they actually or plausibly exist; neither side fiats prevalence.
- THPAW (prefers a world): analyze the likely POINT OF DIVERGENCE between the status quo and a world where X always existed or never existed. Gov does not fiat the divergence point; if Gov squirrels it, propose the more plausible alternative divergence and argue from there. A world without religion is one where religion never arose, not one where believers were forced to give it up; no transition costs exist.
- THR (regrets): retrospective. Construct the counterfactual, show it is LIKELY, and show it is better. The BASELINE CONDITIONS that produced the regretted thing stay constant on both sides: on "THR the global rise in oil prices," the demand surge exists in both worlds and the debate is how it gets satisfied. "Rise of X" or "decline of X" motions debate the TREND, not bare existence: on "THR the rise of dating apps," Gov still defends a world where a few dating apps exist and are used far less.
- THBT: a normative claim judged from a neutral observer's perspective, not from inside any actor. Impacts may land on anyone; the burden is explaining why your impacts matter most.
- POLICY TOPICS (THW, and THBT X SHOULD Y): Gov has fiat over HOW the policy is implemented, never over reactions or compliance. Opp may counter-model a mutually exclusive policy with EQUAL fiat, or defend the status quo. THBT-should is policy, not analysis, with one wrinkle: Gov may choose to run it modelless as an analysis debate, and if Gov presents no model, Opp gets no counter-model.
- ACTOR MOTIONS ("TH, as X, would..."): argue from the actor's knowledge, values, and interests, but the actor is WILLING TO BE PERSUADED by round arguments. Empirical revealed preference is not a trump: on "TH, as Joe Biden, W not run in 2024," Opp cannot auto-win on "he clearly wants to run"; it must show why that preference outweighs his competing interests. Most actors hold multiple competing interests; the round is about which should govern.
- MOTION-TYPE FLAWS TO EXPLOIT (coach cheat-sheet canon): a THR case with no counterfactual is broken, say so and explain the damage; THR teams drifting into future-looking impacts have left the motion; a THW with no model (or a bad one) may not deliver its own benefits; teams treating THPAW like a policy debate have misread the world; actor-motion arguments that never link to the actor's interests are unanchored, call it.

═══ OPPOSITION PLAYBOOK (the five strategies) ═══
- The five Opp strategies: STRAIGHT OPP (defend the opposite of the case statement; the default and the best learning posture), COUNTER-CASE, TIGHT CALL, SPEC CALL, and SQO CALL. The last three are calls about the case itself, not arguments within it.
- COUNTER-CASE RULES: legal only against a policy case, never when the case statement says "preferable to the status quo" or explicitly assigns Opp a position. Equal fiat power to Gov (same money, effort, political capital), and it must be MUTUALLY EXCLUSIVE with Gov's policy. After proposing it, pause and take POCs on it. Never run "case and a cookie" (Gov's policy plus a tweak): it fails to disprove the case statement and loses on that exact line.
- SPEC CALL: the case turns on specific knowledge a reasonable debater would not have. The bar is roughly the front page of the New York Times. Spec targets ARGUMENTS, not whole cases, and is run alongside substantive arguments, never alone. For an unknowable specific, the stronger move is often to assert the opposite: if you do not know it, the judge does not either.
- SQO CALL: status-quo cases are illegal in APDA because Opp would carry the counterfactual burden against infinite prep. A case is not sqo merely because it is true in America, unless the case is specifically about America. Run alongside substance, never alone.
- TIGHT CALL timing discipline: the call belongs in the FIRST 15-30 SECONDS of the LOC. Motions and opp-choice cases can never be tight-called (the taxonomy above governs the rest).
- THEORY POSTURE: APDA theory (tight-call theory, equity theory) exists but the standard counters are always available: it sets bad norms for the league, makes debate inaccessible, and makes rounds worse. Speaks theory (arguing for particular speaker points) is banned everywhere and loses the round outright. Kritiks imported from American high school debate are broadly rejected on APDA; do not run them, and when facing one, note the format norm and engage the motion instead.

═══ CRAFT FROM RECORDED ROUNDS (round + RFD corpus) ═══
- MODEL SETUP SHAPE (PMC): scope the motion to the strong case ("only where these violations are systemic"), anchor scope to an external verifiable standard ("countries appearing year on year on UNHRC and Amnesty lists"), build in reversibility ("we allow them back in if conditions improve"), add a grace period ("improve X metric within a year or the ban lands"), and name enforcement in escalating tiers (fines and lawsuits, then market bans, then license and executive bans). Then: "I'll pause time for model questions."
- BURDEN FRAMING INSIDE THE PMC: state your sufficient win condition before Opp defines it for you ("even a steady improvement in conditions on our side wins us this debate").
- PREEMPTS MUST BE SUBSTANTIVE: preempt the two most predictable Opp arguments with numbered answers, but a throwaway preempt ("Russia has no money because of the war") gets punished by good judges as silly; earn each preempt or cut it.
- INCENTIVE vs CAPACITY SPLIT: deny solvency by splitting the opponent's mechanism into required layers and showing one missing ("even if some individuals have the incentive to stop, no one individual has the capacity to make this decision").
- COUNTERFACTUAL COMPARATIVE, EVERY TIME: "the military gets funded on both sides of the house; on their side they only use the stick." An argument that does not compare both worlds is incomplete; say so in those words.
- CONCEDE-AND-WEAPONIZE: accept the opponent's harm at full strength and make it your link ("recessions do not discriminate, which is exactly why every group in the coalition pressures the regime to reform"). SCALAR version: the worse their impact, the stronger your link ("the worse the capital flight, the more incentive the dictator has to change").
- FACE-SAVING MECHANISM: when claiming a hostile actor complies, hand them a story that lets them comply without losing face ("I reform because the West forces me to").
- CONSISTENCY ATTACK: name when the opponent needs contradictory characterizations ("you cannot have it both ways: the leader can do whatever they want, yet also does exactly what makes him overthrowable").
...
═══ PREP SPINE (coach reading-list canon) ═══
- The prepped-debater knowledge spine, per circuit coaching curricula: macroeconomics and free trade; China's geo-strategic and economic interests; political philosophy of consent and state legitimacy; AI labor effects and alignment; selectorate theory; and demand-side accounts of right-wing populism.
- Econ arguments are mechanism-first and spoken: walk the causal chain aloud (monopsony logic for minimum wages, hot money and capital flight, mortgage securitization, cost-push vs demand-pull inflation, the Volcker Shock's effect on developing-country debt). For development motions: export-oriented vs import-substitution industrialization, SOE privatization, dollarization and de-dollarization.
- Selectorate logic without the jargon: size of the winning coalition explains why autocrats buy loyalty with private goods and democrats buy policy with public goods. Use the logic in any party or authoritarian-regime round; never name-drop the theory.
- Populism motions: run or answer BOTH demand-side accounts, economic insecurity from globalization and immigration-driven cultural backlash, and be able to steelman the pro-populist case.
- Ban-consensual-activity motions (prostitution, drugs, gambling): the consent and state-legitimacy framework is core clash; engage the libertarian arguments against interfering in private life directly rather than pretending harm math settles it.
- Animal motions: the argument from marginal cases is the minimum engagement bar. AI motions: labor displacement mechanisms plus the alignment problem, and be ready to deflate overclaimed "AI destroys democracy" fear. Reaction deadlocks ("how will people actually respond?"): break them with evolutionary priors, explain why one reaction (fear, status-seeking, in-group loyalty) is likelier than the alternative.

═══ INFOSLIDE MOTIONS & MOTIONS-ROUND CRAFT (2026 summer-circuit corpus) ═══
- INFOSLIDE = GROUND TRUTH. Modern motions rounds routinely drop an infoslide with the motion; everything in it is TRUE for the round, the same status as Gov background in a case round. Never contest infoslide facts, never import facts that contradict them; a team arguing against the slide has misplayed the format.
- MINE THE SLIDE FOR THE CRUX. A well-written infoslide hands both teams the intended clash — usually in its final sentence. Real 2026 example: a covered-bonds-vs-MBS slide ends "the credit risk transfers entirely off the bank's balance sheet to those investors"; the round IS "who should hold mortgage credit risk," and the winning material traces originate-to-distribute underwriting decay against cover-pool skin-in-the-game through both normal times and a crisis. Read the slide's last sentence twice before writing anything.
- THE SLIDE SETS SCOPE. On narrow-doctrine motions ("THR the second standard" after a slide on the Brooke Group two-prong predatory-pricing test), the debate is the NAMED provision — the recoupment prong — not predatory pricing generally and not antitrust generally. Stay inside the provision and trace its downstream doctrine; drift to the broader area is off-motion and gets discounted.
- TIMESPACED MOTIONS ("timespaced" is the circuit word). When the motion pins a year ("In 1970, THBT...") or the slide pins an era, argue from knowledge available AT THAT TIME to actors IN THAT WORLD. Hindsight is illegal matter: no arguing a 1970 motion from what Japan became in the 1980s, no arguing a 1947 Perón motion from the 1955 coup. State the temporal scope in your first 30 seconds; "is this timespaced?" is a standard clarification and should never need asking against a competent Gov.
- BEST-INTERESTS-OF-A-PERSON MOTIONS ("THBT it is in [named person]'s best interests to..."): philosophy rounds about one human, NOT policy rounds. The clash lives in wellbeing theory — subjective versus objective accounts of a good life, autonomy versus paternalism, whether meaning built on a false premise counts as meaning, identity continuity (who is the soldier if the war ends?). Societal-stakeholder register ("society benefits when...") is off-motion; the ballot asks about HIS interests only. Win with texture: what his actual days look like in each world, argued at full strength on both sides.
