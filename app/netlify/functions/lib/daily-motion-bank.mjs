...
export const DAILY_MOTIONS = [
  ...
  {
    motion: 'This House would abolish qualified immunity for police officers.',
    domain: 'civic',
    frame: 'Qualified immunity decides whether a person whose rights were violated by police can sue the officer at all. After 2020 it became one of the most contested doctrines in American law, and states have started dismantling it on their own.',
    background: 'Qualified immunity is a judge-made doctrine that shields government officials from lawsuits unless they violated a right already established in a nearly identical prior case, a bar so specific that officers have escaped suits over conduct courts agreed was unconstitutional. Colorado, New Mexico, and New York City moved to strip or bypass it for police after the 2020 protests, and the federal attempt to end it stalled in Congress. Supporters of abolition argue immunity removes the one real deterrent against abuse and leaves victims with no remedy. Opponents argue officers make split-second decisions, that personal liability would make policing impossible to staff, and that cities would simply indemnify every officer anyway, moving the cost to taxpayers rather than changing behavior.',
    govHint: 'A right you cannot sue to enforce is not a right, it is a suggestion.',
    oppHint: 'Strip immunity and either nobody takes the job, or the city pays every judgment and nothing changes but the letterhead.',
  },
  {
  ...
  },
  {
    ...
    background: 'Most US states have shield laws protecting reporter-source confidentiality, but the federal government has none. The NYT, WaPo, and other major outlets have internal standards, typically two independent sources for anonymous attribution, that journalism schools teach but readers cannot verify. The 2003 Jayson Blair scandal and repeated anonymous-source failures in national-security reporting are cited as failures of internal review. Mandatory disclosure would mean filing source agreements with a regulator, a step every major democracy has rejected as state intrusion into the press.',
