    ...
    for (var i = 0; i < n; i++){
      ...
      html += '<div class="rb-row' + (r.rank === 1 ? ' rb-row--top' : '') + '" data-href="/leaderboard" data-rb="leaderboard-row">'
        ...
        + avaCell(r, i, avaUsed, avaTaken)
         + '<span class="rb-who"><a class="rb-nm" href="/users?uid=' + encodeURIComponent(r.uid || '') + '">' + esc(r.name) + '</a>' + (r.publicIdeology ? '<small class="rb-ideology">' + esc(r.publicIdeology) + '</small>' : '') + '<a class="rb-profile-link" href="/users?uid=' + encodeURIComponent(r.uid || '') + '">Profile details →</a>' + (r.rank ? '' : '<small>In placement</small>') + '</span>'
