...
The `ios/` folder now exists (generated via `npx cap add ios`). Steps 1-4
below are DONE on this machine — Xcode 26.6 installed, project generated,
icons/splash dropped in, Info.plist strings added.
...
Bundle ID we're using: **`com.debateai.debateit`** (matches
`capacitor.config.ts` and the Xcode project) — register this exact ID in
