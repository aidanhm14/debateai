...
// Server-side beta no-charge gate — same flag as create-checkout.mjs so one
// env flip (BETA_NO_CHARGE=false) re-enables both Stripe and Razorpay.
// Orders here capture immediately (payment_capture: 1), so this endpoint
// must be closed while beta means no card is collected.
