...
|---|---|---|---|---|
| R01 | Fix the initially indicated area | **Unresolved** | 0:00 to 0:10 | Silent audio; the first pointed-at region cannot be told from the transcript. No guessed change. |
...
| R05 | Redesign the referenced box as an opening | Implemented, medium confidence | `landing.html` walkthrough card | Codex read "this box" (0:55 to 1:05) as the How-it-works opening card and shortened it to "One person. One question. A different point of view." Live. Confidence is medium because the audio is silent. |
...
| R28 | Remove "Type to beat the AI" and its page | **Verified live, scope needs a yes from Aidan** | `netlify.toml`: `/practice` and `/practice.html` 301 to `/newvoice` | The phrase "Type to beat the AI" appears nowhere in the repo history; Codex read the 6:50 to 7:00 frames as the typed `/practice` round and retired that whole surface. That is the largest product change in this batch (soul.md still calls `/practice` the canonical full-round surface). If the intent was one entry point rather than typed practice itself, this is over-scoped and one redirect pair restores it. |
...
4. **R01 and R13 "and number".** Need Aidan to name the region and say whether a vote count was wanted.
