...
> **Homepage Highlights chatter (Aidan, 2026-09-25).** The desktop Highlights panel mixes browser-only scripted conversations from `app/js/landing-chatter.js` into the real highlights for the 80% `on` arm of `landing_chatter_v1`. This is his explicit reversal of the 2026-08-31 no-bots rule for that panel only; do not "fix" it back from an older doc. Never write a scripted line to The Commons, Firestore or `/api/live-chats`; never let a persona match a real account or a generated alias; never let a scripted line reply to a real person or claim anything about the site, judge, ranks, prizes or money. Political lines argue issues: no politician is named and there is no war or election-fraud claim, and a line tied to a date carries an `until` so it retires on its own. New lines must pass `scripts/test-landing-chatter.mjs` (pre-commit), never bypass it. See soul.md, 2026-09-25.

> **September 25 walkthrough completion.** The recorded September 24 corrections retire the public typed `/practice` door into `/newvoice`, including older competitive entry links; the legacy source remains for generators and stored-round compatibility. General AI voice practice remains. Public viewpoint labels are optional, allowlisted profile choices, never inferred from private answers. Ask for a public alias once per account after hydration. Reuse saved age attestations without inventing adulthood or altering transcript/research opt-outs. Keep the first-screen photo rotation faster on the invitation's left side only, and authored “Good chat” examples separate from real verdicts. Dedicated round feedback uses the configured Google Form, separate from formal appeals and general feedback. Preserve the current Watch library while adding parliamentary examples and Watch & vote wording. Voice prompts keep the assigned side, short replies and explicit hypothetical reasoning. See `docs/walkthrough-2026-09-24.md` for bounded testing and its limits.


> **September 24 visual batch.** Training categories have distinct color treatments without pictures. The first walkthrough panel uses the Meet / Choose / Talk sequence; later panels are preserved. Mobile Explore uses a two-column grid, expanding to four columns on tablets, with short destination descriptions. Other walkthrough changes remain in the separate review branch.

> **Round entry and notifications (Aidan, 2026-09-24).** Keep topic and start controls before the collapsed Sound and notifications section. Topic proposals use a native dialog with a scrollable phone layout, preserve text on failure, and close only after the proposal save succeeds. Retrying a match acceptance keeps the same invitation dialog and deadline. Notification setup is explicit on the round, queue and Notifications page; an enabled state requires device registration, not permission alone. Queue-wide alerts remain a separate opt-in. Use service-worker notifications on mobile. Notification clicks focus the exact destination or open another tab, never redirect an unrelated active round. Respect explicit sound mute, recover interrupted audio on later gestures, and discard stale queued cues. Phone setup includes Home Screen instructions; no automatic permission prompt from a timer.
...
> **Round reliability follow-up (2026-09-23).** Camera switches preserve the existing microphone, including mute state. Conversation capture keeps earlier words across pause/rejoin and never silently trims a saved stream; size limits and stalled final saves are explicit errors. Product feedback is independent of transcript-storage consent and lives in a private review inbox. User satisfaction, admin review and independent winner labels remain distinct. Unsaved AI round references are user-reported, not verified evidence. Historical ballots are not automatically rejudged.
...
> **Silent AI button line (Aidan, 2026-09-14).** Keep a visible moving line inside the desktop and mobile "Debate the AI" buttons. It is a visual effect only: no hover audio, automatic spoken invitation or fixed caption overlay. Keep the button text and destination. Reduced motion shows a still line. This supersedes the recorded-invitation requirement below.

> **Accessible topics (Aidan, 2026-09-14).** Suggested topics must be understandable from everyday experience. No fictional-character matchups, franchise lore, celebrity comparisons, sports trivia, gaming knowledge or taste-only rankings of foods, pets and entertainment. Use clear disagreements about ordinary choices, fairness, money, work, relationships, privacy and responsibilities. This supersedes the earlier expansion into playful fandom topics. Apply it to authored pools and AI topic suggestions.

> **Public product simplification (Aidan, 2026-09-13).** Remove betting entirely from public pages, navigation and round controls. Disable new stakes and markets, retain settlement and ledger history for existing stakes, and keep paid voice tokens separate. This supersedes every September 10–11 betting restoration instruction below. Lead with live debates between people, rankings and watching rounds. Keep AI disclosure where it identifies an opponent or explains judging, but remove repeated AI pitches, provider jargon and competing product theses. The homepage three-step explainer is removed. Keep See more and the ORIGINAL expanded page: the walkthrough, product demo, judging section, reviews, trust strip, grouped FAQ and footer as shipped before September 13. The short "Before your first round" FAQ was a mistaken replacement and is removed. Keep the leaderboard visible above the gate and betting removed. Earlier parked chapters stay parked. Remove the added "Debate someone live" headline and its tagline on desktop and mobile. The first screen leads directly with the existing board and actions, without another visible pitch above them. Keep the original "Debate the AI" button on desktop and mobile, including its original copy, hover waveform and recorded voice invitation (`data-ai-invite`, `ai-invite.css`, `ai-invite.js`), as shipped before September 13. Reducing AI marketing does not remove or simplify that choice.
...
"joining the queue is the decision to meet") is REVERTED. Aidan was
dropped into a room with a stranger without ever saying yes, and that is
the empty-chair and no-consent failure the 2026-08-10 ready-check was
built to stop. Every foreground pair lands in `status:'consent'` again
and BOTH people tap accept before a room opens; the background matcher
keeps its accept/decline overlay. Do not remove the acceptance card again
without a fresh decision-log entry from Aidan. If the waiting screen felt
slow, tune the copy or the clock on that card, not the gate.
...
- **Email/password account access is restored and remembered by default** (2026-09-05, Aidan). Generic account choosers offer Google, Apple, and email/password. Password creation, returning sign-in, and password reset must stay reachable. "Keep me signed in" defaults on; turning it off limits the login to the current session. This supersedes the 2026-09-03 removal of public email/password access only. The live-video provider gates below are unchanged.
- **Live video requires Google on web; the iOS app also allows Apple, with no anonymous preview** (2026-09-03; Apple stays in the iOS shell because the App Store mandates it). This includes HUMAN SPECTATORS: Watch rows, the sitewide live card, direct `?spectate=1` links, and the `role:'viewer'` Daily token all require the same provider set. Finished replays remain public. `?stage=1` is the sole exception because it is the non-person rendering feed used by `/air` and OBS; it sends `role:'stage'` and receives the same hidden, receive-only token as a viewer. Email link recovery remains supported; the generic email/password chooser does not grant live-video access. The live-video provider set lives in seven places that must agree: `isLiveVideoAccount()` in `firestore.rules`, `LIVE_VIDEO_PROVIDERS` in `spar-pair.mjs` and `create-daily-room.mjs`, and the client copies in `spar.html`, `js/notifications.js`, `js/live-popup.js`, and `live-round.html`; `scripts/test-spar-google-gate.mjs` pins all seven plus the spectator and stage split. Inside an in-app browser there is no key that turns for live video, so /spar shows the open-in-Safari-or-Chrome instruction with a Copy link button (`?inapp=1` on /spar forces that rendering for QA). Older text below that describes superseded provider sets.
  `/spar`, `spar-pair.mjs`, `matchmaking_queue` create rules, and the
  background "Spar live" pill must agree. Email/password accounts work
  for general account access, and anonymous AI rounds are unchanged. The
