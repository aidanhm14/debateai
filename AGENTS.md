...
- **Web sign-in is Google-only; the iOS app also shows Apple, with no anonymous live-video preview** (2026-09-03; Apple stays in the iOS shell because the App Store mandates it). Email link and password handlers remain dormant for outstanding-link recovery, but no public chooser offers them. The live-video provider set lives in seven places that must agree: `isLiveVideoAccount()` in `firestore.rules`, `LIVE_VIDEO_PROVIDERS` in `spar-pair.mjs` and `create-daily-room.mjs`, and the client copies in `spar.html`, `js/notifications.js`, `js/live-popup.js`, and `live-round.html`; `scripts/test-spar-google-gate.mjs` pins all seven. Inside an in-app browser there is no key that turns for live video, so /spar shows the open-in-Safari-or-Chrome instruction with a Copy link button (`?inapp=1` on /spar forces that rendering for QA). Older text below that describes superseded provider sets.
